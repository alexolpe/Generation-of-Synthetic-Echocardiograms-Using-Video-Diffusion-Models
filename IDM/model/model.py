...
class DDPM(BaseModel):
    def __init__(self, opt):
        ...
        if self.opt['phase'] == 'train':
            self.netG.train() #A: simplement es posa el train mode
    ...
    def feed_data(self, data):
        #A: el getitem del dataset et retorna la data amb el diccionari data['inp'] data['gt']
        data['inp'] = (data['inp'] -self.sub) / self.div
        data['gt'] = (data['gt'] -self.sub) / self.div
        p = random.random() #A: p es un valor random entre 0 i 1

        img_lr, img_hr = data['inp'], data['gt']

        #A: to_pixel_samples retorna dos tensors. el primer de dimensio (hxw, 2) que conte les cordenades x, y de cada pixel i el segon vector es de dimensio (hxw, 3) que conte els valors rgb de cada pixel
        ...
        'cell': cell,
        'gt': img_hr,
        'scaler': torch.from_numpy(np.array([p], dtype=np.float32)) } #A: converteix p en un tensor
    ...
    def test(self, continous=False):
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.parallel.DistributedDataParallel):
                #A: module es per obtenir passar de que self.netG sigui una instancia de DDM a que sigui una instancia de GaussianDiffusion
                self.SR = self.netG.module.super_resolution(
                    self.data, continous) #Aqui peta
            else:
    ...
    def load_network(self):
        ...
        if load_path is not None:
            ...
            if isinstance(self.netG, nn.parallel.DistributedDataParallel):
                network = network.module
            network.load_state_dict(torch.load(gen_path, map_location=torch.device('cpu')), strict=True)
