...
class idm(nn.Module):
    ...
    def forward(self, x, shape, scale1, scale2, shift):
        ...
        cell[:, 0] *= 2 / shape[-2]
        cell[:, 1] *= 2 / shape[-1]
        kk = self.query_rgb(x, scale1, scale2, shift, coord, cell)
        return kk
...
class UNet(nn.Module):
    ...
    def forward(self, x, lr, scaler, time):
        #A: x = [2, 6, 64, 64]
        #A: lr es la 'HR' que s'obte de la LR
        ...
        for i, layer in enumerate(self.ups):
            if isinstance(layer, ResnetBlocWithAttn):
                ...
                x = rearrange(x, 'b (h w) c -> b c h w', h=feats[-1].shape[-2])

        kk = self.final_conv(x)
        return kk
