...
class GaussianDiffusionNP(nn.Module):
    ...
    def sample(self, cond = None, cond_scale = 1., batch_size = 16):
        ...
        return self.p_sample_loop((batch_size, channels, num_frames, height, width), cond = cond, cond_scale = cond_scale)
    ...
    def q_sample(self, x_start, t, noise = None):
        #A: q_sample t'afegeix soroll a les imatges
        #ADUDA: per que afegeix soroll seguint la formula del return?
        noise = default(noise, lambda: torch.randn_like(x_start))

        #A: els extract retornen tensors de dimensio b,1,1,1,1 que contenen alphas random
        #A: tenint en compte la NOTA 1 del WORD, retornem un tensor de dimensions bxcxfxhxw que conte b gifs diferents als quals se'ls hi ha afegit soroll fins a b timesteps diferents
        #A: Es a dir, durant l'entrenament optimitzem termes random de la loss function (L=L0+L1+L2+...+Lt+...+LT). Aixo ho podem fer perque cadascuna de les Lt son funcions tancades
        return (
    ...
    def p_losses(self, x_start, t, cond = None, noise = None, **kwargs):
        b, c, f, h, w, device = *x_start.shape, x_start.device #A: el * es per desempaquetar size i guardar cada valor a la seva variable
        #A: el metode default retorna el primer argument si existeix. Si no, retorna el segon (aixo es el que passara si no es passa l'argument noise al cridar al metode p_losses)
        #A: el fet de posar lambda: ... fa que noise sigui una funcio anonima (realment noise no es una funcio sino que s'executa la funcio anonima (sense nom) que retorna torrch.randn_like(x_start)) que al cridarla, si no existeix noise 1r argument, s'executi torrch.randn_like(x_start)
        #A: torch.randn_like(x_start) retorna un tensor de les mateixes dimensions que x_start ple de valors randoms d'una distribucio estandard normal
        noise = default(noise, lambda: torch.randn_like(x_start))
        
        #A: x_noisy es un tensor amb mateixes dimensions que x_start en el que se li ha afegit soroll fins a b timesteps diferents a cadascun del b gifs que conformen el minibatch en questio
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)

        #A: Aquest if es per si tenim un condicionant de text
        if is_list_str(cond):
            cond = bert_embed(tokenize(cond), return_cls_repr = self.text_use_bert_cls)
            cond = cond.to(device)

        #A: denoise_fn es Unet3D. De fet, estem cridant al metode forward de Unet3D
    ...
    def forward(self, x, *args, **kwargs):
        b, device, height, width = x.shape[0], x.device, self.height, self.width
        #A: ens assegurem que les dimensions de x siguin BatchSize x Channels x Frames x Height x Weight
        check_shape(x, 'b c f h w', c = self.channels, f = self.num_frames, h = height, w = width)
        #A: per que posem (b,)? el metode randint espera com a argument de size un tuple. Si nomes volem una "llista" enlloc d'un tuple, podem utilitzar el truc (b,) per crear una "llista" de longitut b
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        #A: t es un tensor de dimensio b (batch_size) ple de nombres enters (int64 per el .long()) compresos entre 0 i self.num_timesteps
        x = normalize_img(x)
        #A: x es el tensor normalitzat de dimensions BatchSize x Channels x Frames x Height x Weight
...
def video_tensor_to_gif(tensor, path, duration = 120, loop = 0, optimize = True):
    ...
    # create a list of PIL images from the tensor
    # convert PyTorch tensor to numpy array
...
def gif_to_tensor(path, channels = 3, transform = T.ToTensor()):
    img = Image.open(path)
    #A: tuple just useful for the visualization of the elements of the map. Understand a map as a list
    #A: the 2nd argument of map() is the argument of the method located in the 1st argument of map()
    #A: seek_all_images() retorna un objecte en el que es poden iterar els diferents frames del gif img.gif. Cadascun dels frames del gif img.gif es input del metode transform (<=> T.toTensor())
    #A: T.toTensor() fa el seguent: Converts a PIL Image or numpy.ndarray (H x W x C) in the range [0, 255] to a torch.FloatTensor of shape (C x H x W) in the range [0.0, 1.0]
    #A: tensors es una llista de tensors on cada tensor correspon a un frame del gif img.gif. tensors=(tframe1, tframe2, tframe3, ...)
...
class TrainerNP(object):
    def __init__(
        ...
        *,
        ...
        height = diffusion_model.height
        width = diffusion_model.width
    ...
    def siload(self, milestone, **kwargs):
        if milestone == -1:
    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                #A: obs: enlloc d'actualitzar parametres al fer froward backward en 1 minibatch, es fa en "gradient_accumulate_every" minibatches
                
                #A: data es un tensor de dimensions BatchSize x Channels x Frames x Height x Weight
                #A: amb la estructura tensor.to(device) seleccionem la GPU (device) on guardarem el tensor i on s'executaran totes les ordres referents a aquell tensor
                #data = next(self.dl).to(dev)
                data = next(self.dl).to(dev)

                with autocast(enabled = self.amp):#A: autocast es un tema de quants decimals emmagatzemen les variables. En els casos de NN els decimals aporten molta informacio llavors cal activar autocast
                    #A: sabent que self.model es un objecte instancia de la classe GaussianModel (la qual es subclasse de nn.Modul), al fer self.model() estem cridant l metode forward() de ls classe GaussianDiffusion
                    loss = self.model(
                        data,
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                num_samples = self.num_sample_rows ** 2 #A: num_samples=16
                batches = num_to_groups(num_samples, self.batch_size)#A: batches = [4,4,4,4]
                ...
                #A: one_gif es de dim 3x8x4*128x4*128 (4?)
                video_path = str(self.results_folder / str(f'{milestone}.avi'))
                #torch.save(one_gif, str(self.results_folder / f'tensor-{milestone}.pt'))
