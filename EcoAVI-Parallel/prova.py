...
def cosine_beta_schedule(timesteps, s = 0.008):
    ...
    alphas_cumprod = torch.cos(((x / timesteps) + s) / (1 + s) * torch.pi * 0.5) ** (2 * 3)
...
def q_sample(x, t):
    ...
    return x_noisy, t, real_noise, betas
...
#TIFF
original_path = '/data/aolivepe/tiff'
#final_path = '/data/aolivepe/tfg/'

video = []
for nombre_archivo in sorted(os.listdir(original_path)):
    if nombre_archivo.endswith('.tiff'):
        ruta_completa = os.path.join(original_path, nombre_archivo)
...
'''i=0   
joined_path = os.path.join(final_path, f'prova22-{i}.avi')
print(joined_path)
video_tensor_to_gif(final_tensor, joined_path)'''
'''#PNG
...
for i in range(0, 1000, 100):
    t = torch.tensor([i]).long()
    noise_tensor, _, _, betas = q_sample(noise_tensor, t)
    #video_tensor_to_gif(sampled, f'./prova22-{i}.avi')
    video_to_tiff(noise_tensor, final_path, 'cos_tau_3', i)
