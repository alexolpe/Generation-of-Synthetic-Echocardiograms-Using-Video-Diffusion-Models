...
def _handle_path(path, sess, low_profile=False):
    ...
    if path.endswith('.npz'):
        ...
        files = list(path.glob('*.tiff')) + list(path.glob('*.png'))
        if low_profile:
            m, s = calculate_activation_statistics_from_files(files, sess)
        else:
            '''x = np.array([])
            for fn in files:
                kk = imread(str(fn)).astype(np.float32)
                if kk.shape == (64, 64, 3):
                    x = np.append(x, kk)
                    print('kk', kk.shape)'''
            x = np.array([imread(str(fn), pilmode='RGB').astype(np.float32) for fn in files]) #png
            #x = np.array([np.repeat(imread(str(fn))[:, :, np.newaxis], 3, axis=2).astype(np.float32) for fn in files]) #tiff
            print('x si?', x.shape)
...
#/../../../../data/aolivepe/newpreprocessedData/originalgif
 
#/../../../data/aolivepe/newpreprocessedData/frames_originalgif
#SYNTHETIC->/../../../data/aolivepe/REAL_EXPERIMENTS/cosespetites/96x128_cropped_frame
#GROUNDTRUTH->/../../../data/aolivepe/preprocessedData/vr480x640AVI_croped_128x96_frames