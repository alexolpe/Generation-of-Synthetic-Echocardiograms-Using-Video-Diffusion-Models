...
def train(
    ...
    while batch_data is not None:
        ...
        if num_epoch == epoch:
            ...
            si_no=cv2.imwrite(f'./proves/epoch{epoch}-batch_index{batch_index}-slice{0}.png', sr_image)
            print(f"{si_no}-SR image save to .proves/epoch:{epoch}-batch_index:{batch_index}-slice:{0}.png")
            num_epoch+=1
